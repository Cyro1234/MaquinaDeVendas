...
icone_path = relative_to_assets("icone.png")  # Coloque o caminho do arquivo de ícone
...
def inserir():
    ...
    try:
        ...
        valor_inicial += new_inserir

        if valor_inicial > 2.0:
            valor_devolvido = valor_inicial - 2.0
            valor_inicial = 2.0  # Define o valor máximo permitido
            exibir_popup(valor_devolvido)

        canvas.itemconfig(tagOrId=valor_inicial_texto, text=f"R${valor_inicial:.2f}",)
...
valida_comando = (window.register(validar_entrada), '%P')  # '%P' é o novo conteúdo da Entry
...
def liberar_produto():
    ...
    if valor_inicial == 2.0:
        # Escolher uma imagem aleatória entre as 5
        ...
        exibir_popup_bebida()  # Sem imagem, apenas mensagem de valor insuficiente
